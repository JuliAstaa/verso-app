    ...
    <div class="flex flex-col h-full gap-3">
        ...
        @if($isModalOpen)
            @include('sections.admin.categories.modal-form-categories')
        @endif

        <!-- Sections Color-Controls) -->
        <div class="bg-white w-full py-3 px-5 rounded-sm shadow-sm shrink-0">
            @include('sections.admin.categories.categories-control')
        </div>
        ...
        <div class="flex flex-col flex-1 mt-1 bg-white w-full py-3 px-5 rounded-sm shadow-sm overflow-hidden">
                ...
                <div class="w-full overflow-x-auto">
                    <div class="grid grid-cols-4 gap-5 gap-y-10">
                        @forelse($categories as $category)
                            @include('sections.admin.categories.categories-card', ['category' => $category])
                        @empty
                            <div class="col-span-full py-12 text-center text-gray-500 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                                ...
                                <p class="mt-2 font-bold text-gray-900">Belum ada Category</p>
                            </div>
                        @endforelse
                    </div>
                </div>    
            ...
            <div class="mt-4"> 
                {{ $categories->links('vendor.pagination.custome-verso') }}
            </div>
        </div>  
    </div>
